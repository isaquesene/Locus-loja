...
class Chamados_resp_adicionar extends CI_Controller 
{
	function __construct()
	{
        ...
        $tela  = 42; //Cod. da tela - Tabela t_telas
	...
	public function index()
	{
        if(!$_POST)
		{
            $data['msg_status']   = '';
            $data['msg_texto']    = '';  
            $this->load->view('chamados_resp_adicionar', $data);
		}
		else
		{
            $this->adicionar();
	...
	private function adicionar()
	{
		...
		if($this->Chamados_model->adicionar_chamado_resp($dados))
		{
            $data['msg_status']   = 'OK';
            $data['msg_texto']    = 'Resposta Cadastrada com Sucesso. Clique <a href="' . base_url() . 'chamados_resp_gerenciar"><strong>AQUI</strong></a> para voltar a tela anterior.';
            $this->load->view('chamados_resp_adicionar', $data);
		
		}else{
            $data['msg_status']   = 'ERRO';
            $data['msg_texto']    = 'Erro ao Cadastrar Resposta.';  
            $this->load->view('chamados_resp_adicionar', $data);
		}	
