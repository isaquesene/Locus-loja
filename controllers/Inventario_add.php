...
class Inventario_add extends CI_Controller
{
    ...
    public function tempoTemplate()
    {
        ...
        if (isset($_POST["Import"])) {
            ...
            if ($file_type === 'text/csv' || $file_type === 'application/vnd.ms-excel' && $file_ext === 'csv') {
                ...
                if ($divergencia <=0 || $naoMovimentados <=0 || $falteiros <=0 || $total <=0) {

                    $data['msg_status'] = 'Quantidade Invalida';
                    $data['msg_texto'] = 'Os números escolhidos nos parâmetros devem ser maiores do que zero (0).';
                    $this->session->set_flashdata('flash-alert', [$data['msg_status'], $data['msg_texto'], 'error']);
                ...
                array_push($responseAgend,'divergencia' => $divergencia);
                array_push($responseAgend,'naoMovimentados' => $naoMovimentados);
                array_push($responseAgend,'falteiros' => $falteiros);
    ...
    function renova_cesta($divergencia,$naoMovimentados,$falteiros,$responseAgend){
        ...
        unset($cesta);
        //fazer com que essa funcao seja executada a cada insercao de realizacao
        //descobrir o id da cesta criada
    }
        ...
        foreach ($agendamentos as $loja) {
            ...
            switch ($loja['repeticoes']) {
                ...
                case "mensal":
                    ...
                    function datasDeMesEmMes($dataInicio, $dataFim) {
                        $datas = array();
                        $dataAtual = $dataInicio;
                        
                        while ($dataAtual <= $dataFim) {
                            $datas[] = date('Y-m-d H:i:s', strtotime($dataAtual));
                    ...
                    $datas = datasDeMesEmMes($dataInicio, $dataFim);
                    ...
                    $until_target = intval($current_date->diff($target_date)->format("%m"));
                ...
                case "quinzenal":
                    ...
                    function datasDe15em15dias($dataInicio, $dataFim) {
                        $datas = array();
                        $dataAtual = $dataInicio;
                    ...
                    $datas = datasDe15em15dias($dataInicio, $dataFim);
