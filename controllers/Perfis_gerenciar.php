...
class Perfis_gerenciar extends CI_Controller 
{
	function __construct()
	{
        ...
        $tela  = 4; //Cod. da tela - Tabela t_telas
	...
	public function index()
	{
		...
		$this->load->view('perfis_gerenciar', $data);
	}
	
	public function deletar()
	{	
		$per_id = $this->uri->segment(3, 0);		
		if($this->Usuarios_model->deletar_perfil($per_id))
		{
            $data['msg_status']   = 'OK';
            $data['msg_texto']    = 'Perfil Apagado com Sucesso.';  
            
            $data['dados_perfis'] = $this->Usuarios_model->obtem_todos_perfis();
            $this->load->view('perfis_gerenciar', $data);		
		}else{
            $data['msg_status']   = 'ERRO';
            $data['msg_texto']    = 'Erro ao Apagar Perfil.';  
