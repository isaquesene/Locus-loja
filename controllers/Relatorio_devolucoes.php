...
class Relatorio_devolucoes extends CI_Controller 
{
	function __construct()
	{
        ...
        $this->load->model('RelatorioDevolucoes_model');
        ...
        $tela  = 55; //Cod. da tela - Tabela t_telas
    ...
    private function consultar_relatorio(){
        ...
        $data['msg_texto']    = '';
        $data['info'] = false;
        $data['relatorios_filtros'] = null;

        if($_POST){
        $form_data_inicio = $this->input->post('form_data_inicio');
        $form_data_final = $this->input->post('form_data_final');
        $this->session->set_userdata('relatorios-filtros', [
            'form_data_inicio' => $form_data_inicio,
            'form_data_final' => $form_data_final
        ]);
        $data['relatorios_filtros'] = $this->session->userdata('relatorios-filtros');
    }elseif( null !== $relatorioFiltros = $this->session->userdata('relatorios-filtros') ) {
        $form_data_inicio   = $relatorioFiltros['form_data_inicio'];
        $form_data_final    = $relatorioFiltros['form_data_final'];
        $data['relatorios_filtros'] = $relatorioFiltros;
    } else {
        $form_data_inicio   = '';
        $form_data_final    = '';
    }
        ...
        $data['dados_relatorio'] = $this->RelatorioDevolucoes_model->obtem_dados_relatorio($form_data_inicio,$form_data_final);
        $data['info'] = true;
        $this->load->view('relatorio_devolucoes', $data);
    }
    ...
    public function detalhes($id_cbn)
    {
        ...
        $this->load->view('relatorio_devolucoes_detalhes', $data);
    }
}
