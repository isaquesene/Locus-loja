...
class Veiculos_gerenciar extends CI_Controller 
{
	function __construct()
	{
        ...
        $tela  = 6; //Cod. da tela - Tabela t_telas
	...
	public function index()
	{
		...
		$data['dados_veiculos'] = $this->Veiculos_model->obtem_dados_veiculos();
		$this->load->view('veiculos_gerenciar', $data);
	}
	
	public function deletar()
	{	
		$vei_id = $this->uri->segment(3, 0);		
		if($this->Veiculos_model->deletar_veiculo($vei_id))
		{
            $data['msg_status']   = 'OK';
            $data['msg_texto']    = 'Veiculo Apagado com sucesso.';  
            
            $data['dados_veiculos'] = $this->Veiculos_model->obtem_dados_veiculos();
            $this->load->view('veiculos_gerenciar', $data);		
		}else{
            $data['msg_status']   = 'ERRO';
            $data['msg_texto']    = 'Erro ao Apagar Veiculo.';  
