...
class Nf_model extends CI_Model
{
    ...
    function atualiza_nf_para_nao_entregue($tempo) {
        ...
        if ($total > 0) {
            if ($total > 100) {
                // Atualiza múltiplas linhas com uma única consulta
                $this->db->set('cbn_status', 'NÃO ENTREGUE');
                $this->db->where("CONCAT(cbn_data_emissao, ' ', cbn_hora_emissao) < '$data_limite'");
                $this->db->where_in('cbn_status', ['FATURADA', 'EM TRANSITO']);
                $this->db->update('t_cabecalho_notas');
            } else {
                // Seleciona o cbn_id das linhas que atendem aos critérios de seleção
                ...
                // Cria um array com os cbn_id para ser usado na cláusula where_in
                $cbn_ids = [];
                foreach ($query->result() as $row) {
                    $cbn_ids[] = $row->cbn_id;
                }
    
                // Atualiza as linhas com uma única consulta
                if (!empty($cbn_ids)) {
                    $data = array('cbn_status' => 'NÃO ENTREGUE');
                    $this->db->where_in('cbn_id', $cbn_ids);
