...
class Conferencia_externa extends CI_Controller 
{
	...
	public function index()
	{
        ...
        $data['justificativas'] = $this->Conferencia_model->obtem_justificativas();        
    ...
    public function buscar_nota(){               
        ...
        $recusa = $this->input->post('recusa');
        $recusa_justificativa = $this->input->post('recusa_justificativa');

        $valida = $this->Conferencia_model->valida_nota($chave);

        $validaInterna = $this->Conferencia_model->valida_nota_interna($chave);

        $nota_finalizada = $this->Conferencia_model->verifica_nota_finalizada($chave);

        $nota_cancelada = $this->Conferencia_model->verifica_nota_cancelada($chave);

        $nota_recusada = $this->Conferencia_model->verifica_nota_recusada($chave);

        if($nota_recusada){
            
            $data['msg_status']   = 'Atenção';
            $data['msg_texto']    = 'Nota já se encontra Recusada.';
        ...
        if($nota_cancelada){
            ...
            $data['msg_status']   = 'Atenção';
            $data['msg_texto']    = 'Nota se encontra Cancelada.';
            $this->session->set_flashdata('flash-alert', [$data['msg_status'], $data['msg_texto'],'warning']);   
        ...
        if($nota_finalizada){
            ...
            $data['msg_status']   = 'Atenção';
            $data['msg_texto']    = 'Nota fiscal já finalizada.';
            $this->session->set_flashdata('flash-alert', [$data['msg_status'], $data['msg_texto'],'warning']);   
        ...
        if ($valida == false) {
            ...
            $data['msg_status']   = 'Atenção';
            $data['msg_texto']    = 'Nota já se encontra Entregue';
            $this->session->set_flashdata('flash-alert', [$data['msg_status'], $data['msg_texto'],'warning']);   
        ...
        elseif($uso <= 0){
            ...
            elseif ($matching->mat_sit_nfe == 2 && $matching->mat_num_pedido == 0) {
                ...
                $msg = 'Nota fiscal pendente de Matching.';
                $this->session->set_flashdata('flash-alert', ['Erro', $msg, 'error']);
            ...
            if(isset($recusa) && $recusa == 'recusada'){
                ...
                $usuario_recusa_inicial = $nivel = $this->session->userdata('usu_id');
                
                $this->Conferencia_model->alterar_status_nota_externa($chave,$recusa,$recusa_justificativa,'','',$usuario_recusa_inicial);

                $data['msg_status']   = 'NOTA RECUSADA';
                $data['msg_texto']    = 'A nota foi recusada.';
                $this->session->set_flashdata('flash-alert', [$data['msg_status'], $data['msg_texto'],'success']);   
    
                redirect(base_url() . 'conferencia_externa_recusa');
